...
def remove_background(
    ...
    """
    Remove the background from an image using a 2D background model.

    Args:
        image: The input image from which to remove the background.
        header: The FITS header associated with the image.
        file_path: The path to the file where the background-subtracted image will be saved.
        bw: The width of the background estimation box.
        bh: The height of the background estimation box.
        estimator: The background estimator to use.
        save_file: Whether to save the background-subtracted image to a file.
    ...
    out_path = p.with_stem(f'{p.stem}_bkg_sub')
...
def find_stars(tile: str, header: Header) -> pd.DataFrame | None:
    """
    ...
    Returns:
    - star_df: DataFrame containing information about the stars found in and around the field.
    """
    wcs = WCS(header)
    pixel_scale = abs(header['CD1_1'] * 3600)  # type: ignore
...
def star_fit(df_if: pd.DataFrame, survey: str = 'UNIONS') -> pd.DataFrame:
    """
    Fit star properties to the input DataFrame.

    Args:
        df_if: Input DataFrame containing star information.
        survey: Survey name to determine fitting parameters.

    Returns:
        Fitted DataFrame with additional columns for star properties.
    """
    df = df_if.copy()
    # fit star size
    if survey == 'UNIONS':
...
def mask_stars(
    ...
    if save_to_file:
        p = Path(file_path)
        out_path = p.with_stem(f'{p.stem}_star_masked')
...
def streak_mask(data_masked, file_path, bkg_rms, table_dir, header, psf_multiplier=2.0):
    ...
    if len(streaks.index[streaks['real_streak'] == 1] > 0):
        ...
        logger.info(f'Streaks masked in tile {Path(file_path).name}.')
    else:
        logger.info(f'No streaks found in tile {Path(file_path).name}.')
...
def smooth_image(data_masked, file_path, header, psf_multiplier=2.0):
    ...
    """
    logger.info(f'Smoothing tile {Path(file_path).name}..')
    ...
    logger.info(
        f'Kernel for tile {Path(file_path).name} created. Took {np.round(time.time() - kernel_start)} seconds.'
    )
    data_conv = convolve(data_masked, kernel)
    hdu_new = fits.PrimaryHDU(data_conv, wcs.to_header())
    out_file = (
        Path(file_path).with_stem(f'{Path(file_path).stem}_star_masked_smooth').with_suffix('.fits')
    )
    hdu_new.writeto(out_file, overwrite=True)
    logger.info(
        f'Tile {Path(file_path).name} smoothed. Took {np.round(time.time() - smoothing_start)} seconds.'
    )
    pass
...
) -> tuple[np.ndarray, Path, Header]:
    # out path
    p = Path(file_path)
    out_path = p.with_stem(f'{p.stem}_{preprocess_type}')
    # wcs
    wcs = WCS(header)
...
def prep_tile(
    ...
    band: str,
    bin_size: int = 4,
) -> tuple[np.ndarray, np.ndarray, Path, Header]:
    """
    ...
    Args:
        ...
        file_path_binned: path to the binned fits file
        fits_ext: fits extension, 0 or 1
        zp: photometric zeropoint for this tile
        band: band of the image, e.g. 'cfis_lsb-r', 'whigs-g'
        bin_size: bin factor, defaults to 4
    Returns:
        str, str: path to preprocessed file, header of preprocessed file
    """
    prep_start = time.time()
    # only rebin full res image if binned file does not exist
    if not file_path_binned.is_file():
        ...
        data_binned, header_binned = open_fits(file_path_binned, fits_ext=0)
    start = time.time()
    ...
    try:
        ...
        if objects_sep is None or data_sub_sep is None or bkg_sep is None or segmap_sep is None:
            raise Exception('source_detection failed after multiple attempts')

        # get photometry of detected objects
        pixel_scale = abs(header_binned['CD1_1'] * 3600)  # type: ignore
...
def aperture_photometry_mag_auto(
    data_sub, objects, zp=30.0, pixel_scale=0.7431
...
def mask_small_objects(
    ...
    grid_size: int = 25,
    save_file: bool = False,
) -> tuple[np.ndarray, np.ndarray, pd.DataFrame, np.ndarray, np.ndarray, np.ndarray]:
    if bkg_mean is None:
        # Create background statistics map
    ...
    very_small_object_ids = objects_df.loc[very_small_objects_mask, 'ID'].to_numpy()
    medium_object_ids = objects_df.loc[medium_objects_mask, 'ID'].to_numpy()
    large_object_ids = objects_df.loc[large_objects_mask, 'ID'].to_numpy()  # noqa: F841
    ...
    if save_file:
        out_path_tiny = file_path.with_stem(f'{file_path.stem}_tiny')
