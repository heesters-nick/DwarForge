...
LIB_PATH = HERE / 'lib' / 'mt_objects.so'
if not LIB_PATH.exists():
    raise FileNotFoundError(f'MTO shared library not found at {LIB_PATH}')
# mto_lib = ct.CDLL('mtolib/lib/mt_objects.so')
mto_lib = ct.CDLL(str(LIB_PATH))
...
def filter_tree(mt_in, image, params, sig_test=default_sig_test, sig_nodes_function=up_tree):
    if params.verbosity:
        print('\n---Finding Objects---')
...
def filter_tree_timed(mt_in, image, params, sig_test=default_sig_test, sig_nodes_function=up_tree):
